...
def saada_kiri():
    ...
    kiri=kiri_box("1.0",END)
    smtp_server="smtp.gmail.com"
    port=587
    sender_email="kagaponenko@gmail.com"
    password="а c q z h g p l e m d n j t a"    ####
    context=ssl.create_default_context()
    msg=EmailMessage()
    msg.set_content(kiri)
    msg['Subject']="E-kiri saatmine"
    msg['From']="Ekaterina Gaponenko"
    msg['To']=kellele
    with open("vihm.png",'rb') as fpilt:
        pilt=fpilt.read()
...
email_box=Entry(aken,bg="#e7efe3",fg="#38761d",font="Arial 20",width=20,justify=LEFT)
kiri_box=Text(aken,bg="#e7efe3",fg="#38761d",font="Arial 20",height=6,width=20)
saada_btn=Button(aken,text="Saada",bg="#38761d",fg="#e7efe3",font="Castellar 16",relief=RAISED,command=saada_kiri)

pilt_btn=Button(aken,text="Pilt",bg="#38761d")

l_mail.grid(row=0,column=0)
l_kiri.grid(row=1,column=0)
email_box.grid(row=0,column=2,columnspan=2)
kiri_box.grid(row=1,column=2,columnspan=2)
